<div class="discount-box" data-order-id="{{ $active_table->id }}">
    ...
    <div class="row">
        <div class="col-6">
            ...
            <select class="form-control discount_type">
                ...
                <option value="percent" {{ ($active_table->discount_type ?? '') === 'percent' ? 'selected' : '' }}>نسبة %</option>
                <option value="fixed" {{ ($active_table->discount_type ?? '') === 'fixed' ? 'selected' : '' }}>قيمة ثابتة</option>
            </select>
        </div>

        <div class="col-6">
            <label>قيمة الخصم</label>
            <input type="number" class="form-control discount_value" min="0" value="{{ $active_table->discount ?? 0 }}">
        </div>
    </div>
    ...
    <br>
     <h6 style="background-color: cornflowerblue;">
        ...
        @php
            ...
            $drinksPrice = $active_table->orderItems->sum('total_cost');
            $total       = $timePrice + $drinksPrice;

            $discount_type  = $active_table->discount_type ?? null;  // النوع
            $discount_value = $active_table->discount ?? 0;          // القيمة المدخلة
