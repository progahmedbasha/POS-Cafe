...
<div class="modal fade" id="roomModal{{$index}}" tabindex="-1" aria-labelledby="roomModalLabel{{$index}}"
    ...
    @php
        ...
        $cleanPhone = ltrim($active_room->client->phone, '0');
        $clientPhone = '20' . $cleanPhone;

        $message = "🧾 *رقم الفاتورة :* " . $active_room->number . "\n";
        $message .= "📍 *الخدمة:* " . $active_room->service->name . "\n";
        $message .= "--------------------------\n";

        // تفاصيل الطلبات (المشروبات)
        if ($active_room->orderItems->count() > 0) {
            foreach ($active_room->orderItems as $item) {
            ...
            $message .= "--------------------------\n";
            $drinksTotal = $active_room->orderItems->sum('total_cost');
            $message .= "🥤 *إجمالي المشروبات:* $drinksTotal ج\n";
        }

        // وقت الوقت
        $timeStarted = $active_room->orderTimes[0]->start_time ?? null;
        $timeEnded = $active_room->orderTimes[0]->end_time ?? null;
        ...
        // إجمالي المبلغ
        if ($active_room->orderItems->count() > 0 && $timeEnded) {
